...
// alloco la memoria in map attraverso insert
    ...
    class map
    {
        ...
        public:
                ...
                void insert( const value_type& value )
                {
                   ...
                   /* if (_size == 1)
                    {
                        _map = _allocation.allocate(1);
                        _allocation.construct(_map, _tree.root->data);
                    }
                    else
                    {
                        _allocation.deallocate(_map, _size);
                        _map = _allocation.allocate(_size);
                        for (size_t i = 0; i < _size; i++)
                        {
                            _allocation.construct(&_map[i], _tree.inorderHelper());
                        }
                    }
                    */
                }
...
int main()
{
  /*  std::pair<std::string, std::string>     pair_val;
    ...
    ft::RBTree<std::pair<std::string, std::string> > tree;
   ...
   std::pair<int, int>     pair_val4;
    pair_val4 = std::make_pair(10, 9);

    std::pair<int, int>     pair_val5;
    pair_val5 = std::make_pair(20, 3);
